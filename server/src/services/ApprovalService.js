...
class ApprovalService {
   ...
   */
  static async getNextStage(currentStageId, client = pool) {
  ...
  static async approve(ecoId, approverId, approverName) {
    ...
    try {
      ...
      const nextStage = await this.getNextStage(eco.stage_id, client);
  ...
  /**
   * Rejects an ECO and sets its status to 'rejected' with optional reason.
   ...
   */
  static async reject(ecoId, approverId, approverName, reason = '') {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      ...
      // 3. Update the ECO status to 'rejected' and set rejection reason
      await client.query(
        "UPDATE ecos SET status = 'rejected', rejection_reason = $1, updated_at = NOW() WHERE id = $2",
        [reason || null, ecoId]
      );
